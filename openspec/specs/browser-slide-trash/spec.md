...
Defines a per-workspace Trash tier in the browser Forge tool: discarded slide files are moved into workspace-scoped trash rather than deleted outright, are listed in a dedicated sidebar group, can be restored back to the live "Slides" list, and can be permanently removed via an explicit "empty trash" action.
## Requirements
