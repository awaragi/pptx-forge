# browser-help-modal Specification

## Purpose
Give new users of the browser tool (`pptx-forge.html`) in-app orientation — what a workspace is, how the editor and import/export work, and how the AI copy/paste bridge works — without requiring a build dependency or an external docs site. The help modal is reachable at any time via a `?` button in the topbar and, on true first-ever visit, opens automatically once to surface the tool's local-only execution guarantee up front.
