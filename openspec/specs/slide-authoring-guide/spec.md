...
- `accentBlock(slide, box, { bgColor, border?, accent, title?, titleColor? }, opts, name)` — returns `AccentBlockRegion { ix, iw, contentY }` for placing content inside
...
#### Scenario: AI uses accentBlock return value for inner content
- **WHEN** an AI uses `comp.accentBlock`
- **THEN** it captures the return value and uses `region.ix`, `region.iw`, `region.contentY` to position text inside the block
