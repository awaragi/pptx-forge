...
### Requirement: Source split into seven modules
The `src/` directory SHALL contain exactly these files after the change: `theme.js`, `primitives.js`, `components.js`, `tables.js`, `layout.js`, `frame.js`, `lib.js` (factory), `index.js` (barrel). Each file SHALL contain only the concerns named by its filename.

#### Scenario: All module files exist in src/
- **WHEN** the `src/` directory is listed
- **THEN** it contains `theme.js`, `primitives.js`, `components.js`, `tables.js`, `layout.js`, `frame.js`, `lib.js`, and `index.js`

#### Scenario: theme.js contains defaultTheme and deepMerge
- **WHEN** `src/theme.js` is inspected
- **THEN** it exports `defaultTheme` and `deepMerge` and contains no function implementations for slide-building primitives or components

#### Scenario: lib.js is the factory assembler
- **WHEN** `src/lib.js` is inspected
- **THEN** it imports from all sibling modules and exports `createLib` — it contains no component logic itself
...
### Requirement: Existing src/lib.js import path remains valid
`src/lib.js` SHALL continue to export `createLib` as a named export. Workspaces that currently import `{ createLib } from '../../src/lib.js'` SHALL work without modification.

#### Scenario: Named import from src/lib.js still resolves
- **WHEN** a file does `import { createLib } from '../../src/lib.js'`
...
- **THEN** the returned object contains `theme`, `run`, `prim`, `comp`, `tables`, `layout`, and `frame`
