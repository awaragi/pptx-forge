...
### 10. `pptxviewjs` is loaded via a lazy `import('pptxviewjs')` inside `getViewer()`, not a static top-of-file import (found during implementation)
`chart.js/auto` self-registers its entire controller/scale/plugin set as a side effect of being imported, at real, measurable cost. `preview.js` is imported unconditionally by `app.js` on every page load, so a static `import { PPTXViewer } from 'pptxviewjs'` at the top of the module paid that cost on every load/reload — including pages that never render a preview at all. This wasn't just an abstract "bundle size" concern: it measurably slowed page bootstrap (~560ms → ~3.6s per test in the e2e suite) enough to make an unrelated, timing-sensitive existing test (`drag-drop-import.spec.js`'s zip-import assertions) fail under repeated/back-to-back runs, with no thrown error — the import simply hadn't finished applying by the time the test's assertion timeout elapsed. Switching to a dynamic import inside `getViewer()` (only called once `updatePreviewNow()` has an actual slide to render) defers `chart.js`'s registration to the first real preview compile; esbuild still inlines the module into the single-file bundle (no code-splitting, `format: iife`), so download size is unaffected, but evaluation is deferred. Verified: the same repeated-run reproduction that reliably failed now passes at the original (pre-this-change) speed.

### 11. The canvas's inline size is cleared before every re-render (found during a post-implementation bug report)
`pptxviewjs`'s `slideSizeMode: 'fit'` measures the canvas's CSS box once per `render()` call, then locks that measurement in as a fixed inline `style="width:...px;height:...px"` on the canvas element — which then overrides this project's own CSS (`width:100%;height:100%` on `#preview-canvas`) permanently. Every later measurement, even from a fresh `render()` call, just reads back the same frozen inline value instead of the wrapper's real current size — confirmed by instrumenting the canvas and observing the exact same pixel dimensions survive a viewport resize, a divider drag, and even a full recompile-and-reselect. Fixed by explicitly clearing the inline `width`/`height` style (`element.style.removeProperty(...)`) immediately before every `render()` call — in the normal compile path, on a new debounced `window resize` listener that wasn't present before (window resizes previously never triggered any re-render at all), and during drag-resize.

The first drag-resize attempt only cleared the inline style on every `pointermove` without a real re-render, relying on CSS to stretch the existing bitmap live — cheap, but visibly distorted the aspect ratio as the drag progressed (a real user-reported issue). Replaced with a `requestAnimationFrame`-throttled **real** re-render on every `pointermove`: at most one actual `render()` call per painted frame, always using the latest pointer position by the time it fires, so intermediate frames during a drag are correctly proportioned instead of stretched. Empirically verified acceptable performance: a synthetic 40-step drag produced 40 real bitmap re-renders in ~670ms (~60fps), i.e. `render()` comfortably keeps pace with a real drag without falling behind or introducing lag.
...
- **A static import of `pptxviewjs` would pay `chart.js/auto`'s registration cost on every page load**, even when no preview is ever rendered, measurably slowing page bootstrap. → Mitigation: lazy `import('pptxviewjs')` deferred to first actual use (Decision 10); confirmed the previously-destabilized `drag-drop-import.spec.js` now passes reliably under repeated runs.
