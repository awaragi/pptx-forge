...
- `tables.dataTable`/`comparisonTable`, and any `addTable`/`addMedia`-shaped object, inside a master. pptxgenjs's `SlideMasterProps.objects` union has no `table`/`media` variant at all.
